...
const CurationFeed = ({ children, feed, setFeed }) => {
    ...
    return (
        <div className="flex md:flex-col md:justify-end gap-4 font-satoshi font-semibold w-full overflow-x-scroll">
            {data.map((item, index) => (
                <div key={index} className="flex flex-col flex-shrink-0 gap-4 w-full p-4 border border-gray-500 rounded-md bg-black/50 hover:bg-white hover:text-black cursor-pointer">
