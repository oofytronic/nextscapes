...
const CurationFeed = ({ children, feed, setFeed }) => {
    ...
    const data = [
        ...
        { title: 'Stereogum', url: 'https://www.stereogum.com/feed/' },
    ...
    return (
        <div className="flex md:flex-col md:justify-end gap-4 font-satoshi font-semibold w-full overflow-x-scroll">
            {data.map((item, index) => (
                <div key={index} className="flex flex-col flex-shrink-0 gap-4 p-4 border border-gray-500 rounded-md bg-black/50 hover:bg-white hover:text-black cursor-pointer">
                    <div className="flex flex-shrink-0 items-center gap-2">
                        <FeedIcon siteUrl={item.url} />
                        <p className="font-bold text-md" onClick={() => handleClick(index, item.url)}>
                            {item.title}
                        </p>
                    </div>
                    {activeIndex === index && (
